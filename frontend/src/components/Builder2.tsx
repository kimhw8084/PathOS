  ...
  const renderHistorySurface = () => {
    return (
      <div className="space-y-4 animate-apple-in pb-10">
        <div className={cn(BUILDER_PANEL, "p-3 space-y-3")}>
          ...
          {workflowDiff.addedTasks.length > 0 && (
            <div className="space-y-2">
              ...
              {workflowDiff.addedTasks.map((task: any) => (
                <div key={`added-${task.node_id || task.id}`} className="rounded-[1.15rem] border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-[11px] font-bold text-emerald-100">
                  <div className="flex items-center justify-between gap-3">
          ...
          {workflowDiff.removedTasks.length > 0 && (
            <div className="space-y-2">
              ...
              {workflowDiff.removedTasks.map((task: any) => (
                <div key={`removed-${task.node_id || task.id}`} className="rounded-[1.15rem] border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-[11px] font-bold text-rose-100">
          ...
          {workflowDiff.addedEdges.length > 0 || workflowDiff.removedEdges.length > 0 || workflowDiff.changedEdges.length > 0 ? (
            <div className="space-y-2">
              ...
              {workflowDiff.addedEdges.map((edge: any) => (
                <div key={`edge-added-${edge.id}`} className="rounded-[1.15rem] border border-sky-500/20 bg-sky-500/10 px-4 py-3 text-[11px] font-bold text-sky-100">
                ...
                <div key={`edge-${edge.id}`} className="rounded-[1.15rem] border border-sky-500/20 bg-sky-500/10 px-4 py-3 text-[11px] font-bold text-sky-100">
                  ...
                  <div className="mt-2 space-y-1">
                    {edge.changedFields.map((field: any) => (
                      <div key={`${edge.id}-${field.key}`} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                        <p className="text-[8px] font-black uppercase tracking-[0.18em] text-white/40">{field.key.replace(/_/g, ' ')}</p>
                        <p className="mt-1 text-[10px] font-bold text-white/70">
                          {formatDiffValue(field.before)} <span className="text-white/25">→</span> {formatDiffValue(field.after)}
                        </p>
              ...
              {workflowDiff.removedEdges.map((edge: any) => (
                <div key={`edge-removed-${edge.id}`} className="rounded-[1.15rem] border border-sky-500/20 bg-sky-500/10 px-4 py-3 text-[11px] font-bold text-sky-100">
        ...
        {rollbackPreview?.available && onCreateRollbackDraft && (
          <button onClick={onCreateRollbackDraft} className="w-full rounded-[1.15rem] border border-theme-accent/20 bg-theme-accent/10 px-4 py-3 text-[10px] font-black uppercase tracking-[0.18em] text-theme-accent hover:bg-theme-accent hover:text-white transition-all">
    ...
    <div className="flex h-full min-h-0 w-full bg-[#050914] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 relative overflow-hidden">
        ...
        {layoutPrefs.inspectorCollapsed ? (
          ...
          <div ref={taskPaneScrollRef} className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar p-3 sm:p-4">
          {selectedTaskId && selectedTask ? (
            <div className={cn(taskPaneCompact ? "space-y-3" : "space-y-4", "animate-apple-in")}>
              <div className={cn(BUILDER_PANEL, "p-3 space-y-3")}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <div className="min-w-[106px] rounded-[1.15rem] border border-white/10 bg-black/25 px-3 py-2">
                      <div className="flex items-center justify-between gap-3">
                    ...
                    <button onClick={() => setTaskPaneCompact((prev) => !prev)} className="rounded-[1.15rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                      {taskPaneCompact ? 'Expand Pane' : 'Compact Pane'}
                    </button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setUtilityPane('comments')} className="rounded-[1.15rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-theme-accent">
                  ...
                  <button onClick={openHistoryPane} className="rounded-[1.15rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                    History
                  </button>
                  <button onClick={() => selectedTaskNode && setCenter((selectedTaskNode.position?.x || 0) + 160, (selectedTaskNode.position?.y || 0) + 120, { zoom: 1.05, duration: 250 })} className="rounded-[1.15rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                    Center
                  </button>
                  <button onClick={() => {
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <button onClick={() => scrollToTaskSection('overview')} className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Overview</button>
                  <button onClick={() => scrollToTaskSection('dependencies')} className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Dependencies</button>
                  <button onClick={() => scrollToTaskSection('ownership')} className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Ownership</button>
                  <button onClick={() => scrollToTaskSection('validation')} className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Validation</button>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-white/10 bg-black/25 px-3 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/45">
              ...
              {inspectorTab === 'overview' && (
                <div ref={(el) => { taskSectionRefs.current.overview = el; }} className={cn(taskPaneCompact ? "space-y-4" : "space-y-6")} data-section="overview">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      <div className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2">
                  ...
                  <div className="rounded-[1.15rem] border border-white/5 bg-white/[0.02] p-3 space-y-3">
                    ...
                    {selectedTaskDiff.changedFields.length === 0 ? (
                      <div className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2.5 text-[11px] font-bold text-white/35">
                        No task-level field changes detected.
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {selectedTaskDiff.changedFields.slice(0, 4).map((field) => (
                          <div key={field.key} className="rounded-[1.15rem] border border-white/10 bg-black/20 px-3 py-2">
                            <p className="text-[8px] font-black uppercase tracking-[0.18em] text-white/40">{field.key.replace(/_/g, ' ')}</p>
                            <p className="mt-1 text-[10px] font-bold text-white/70">
                              {formatDiffValue(field.before)} <span className="text-white/25">→</span> {formatDiffValue(field.after)}
                            </p>
                  ...
                  {!isProtected && selectedTask.task_type !== 'LOOP' && (
                    <>
                      ...
                      <div className="flex items-center justify-between gap-3 px-1" ref={(el) => { taskSectionRefs.current.systems = el; }} data-section="systems">
                        <div>
                          ...
                          <button
                            ...
                            className={cn("rounded-[1.15rem] border px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em]", selectedTaskDiagnostics.taskProfile?.shadow_it_used ? "border-amber-500/20 bg-amber-500/10 text-amber-300" : "border-white/10 bg-white/5 text-white/45 hover:text-white")}
                        >
