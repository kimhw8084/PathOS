...
const getDefinitionIssueShell = (hasIssue: boolean) => cn(
  "rounded-[0.9rem] border bg-white/5 p-[3px]",
  hasIssue ? "border-status-error/40 bg-status-error/10" : "border-white/10",
);

const BUILDER_RADIUS = 'rounded-[0.9rem]';
const BUILDER_PANEL = 'rounded-[0.9rem] border border-white/10 bg-white/[0.03] shadow-[0_24px_60px_-40px_rgba(0,0,0,0.8)]';
const BUILDER_FIELD = 'w-full rounded-[0.9rem] border border-white/10 bg-black/30 px-2 py-1.5 text-[10px] text-white outline-none focus:border-theme-accent focus-visible:ring-2 focus-visible:ring-theme-accent/25 focus-visible:ring-offset-0';
const BUILDER_BUTTON = 'rounded-[0.9rem] px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.16em] leading-none transition-all';
          ...
          <div className="flex items-center gap-2">
            ...
            {isConfirming ? (
              ...
              <button 
                ...
                className="opacity-0 group-hover/item:opacity-100 p-1.5 hover:bg-status-error/20 text-white/20 hover:text-status-error transition-all rounded-[0.9rem]"
              >
                <Trash size={12} />
              </button>
            )}
...
const MatrixNode = ({ data, selected }: { data: any, selected: boolean }) => {
  ...
  if (isTemplate) {
    return (
      <div className={cn(
        ...
        <div className={cn("absolute -top-2.5 left-4 px-2 py-0.5 rounded-[0.9rem] text-[8px] font-black uppercase tracking-[0.2em] border z-20 shadow-lg", isTrigger ? "bg-cyan-500 border-cyan-400 text-white" : "bg-rose-500 border-rose-400 text-white")}>
  ...
  return (
    <div className={cn(
      `apple-glass !bg-[#0f172a]/95 !${BUILDER_RADIUS} px-3.5 sm:px-3 py-2.5 sm:py-4 w-[clamp(300px,31vw,430px)] max-w-[92vw] shadow-2xl transition-all duration-300 relative border-2 h-auto min-h-[300px] hover:z-[1000]`,
      ...
      {data.validation_needed && (
        <div className="absolute -top-2.5 right-4 px-2 py-0.5 rounded-[0.9rem] text-[8px] font-black uppercase tracking-[0.2em] bg-orange-500 border border-orange-400 text-white z-20 shadow-lg animate-pulse">
          VALIDATION REQUIRED
        </div>
      )}
      {data.commentSummary?.total > 0 && (
        <button
          ...
          className="absolute -top-2.5 right-4 flex items-center gap-1.5 rounded-[0.9rem] border border-white/10 bg-[#0b1221]/96 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/60 shadow-lg transition-colors hover:text-white"
      ...
      <div className="flex flex-col gap-2.5 h-full">
        <div className="flex items-center justify-between gap-2">
          <div className={cn("px-2.5 py-0.75 rounded-[0.9rem] text-[10px] font-black uppercase tracking-widest border", typeColor)}>
            {data.task_type || 'GENERAL'}
          </div>
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {data.occurrence > 1 && (
              <div className="flex items-center gap-1 bg-blue-500 text-white px-2 py-0.75 rounded-[0.9rem] text-[10px] font-black shadow-lg shadow-blue-500/20">
                <RefreshCw size={12} /> {data.occurrence}
              </div>
            )}
            {data.blockerCount > 0 && (
              <div className="flex items-center gap-1 bg-amber-500 text-white px-2 py-0.75 rounded-[0.9rem] text-[10px] font-black shadow-lg shadow-amber-500/20">
                <AlertCircle size={12} /> {data.blockerCount}
              </div>
            )}
            {data.errorCount > 0 && (
              <div className="flex items-center gap-1 bg-status-error text-white px-2 py-0.75 rounded-[0.9rem] text-[10px] font-black shadow-lg shadow-status-error/20">
        ...
        <div className="flex flex-col gap-2.5 mt-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="bg-black/40 rounded-[0.9rem] p-2 border border-white/5 flex flex-col items-center justify-center">
      ...
      {selected && (
            <div className="absolute left-1/2 top-full mt-1.5 z-30 flex -translate-x-1/2 flex-wrap justify-center gap-1.5 pointer-events-auto">
              <button onClick={(event) => { event.stopPropagation(); data.onOpenComments?.(data.id); }} className="flex items-center gap-2 rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-theme-accent shadow-lg shadow-black/30">
                <MessageSquare size={10} /> Comment
              </button>
              {data.commentSummary?.total > 0 && (
                <button onClick={(event) => { event.stopPropagation(); data.onOpenComments?.(data.id); }} className="flex items-center gap-2 rounded-[0.9rem] border border-white/10 bg-white/5 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 shadow-lg shadow-black/30">
...
const DiamondNode = ({ data, selected }: { data: any, selected: boolean }) => {
  ...
  return (
    ...
    {data.validation_needed && (
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[112px] px-1.5 py-0.5 rounded-[0.9rem] text-[6px] font-black uppercase bg-orange-500 border border-orange-400 text-white z-30 shadow-lg animate-pulse">VALID</div>
    )}
  </div>
);
};
...
const CustomEdge = ({
  ...
  return (
    <>
      ...
      {data?.label && (
        <EdgeLabelRenderer>
          <div style={{ position: 'absolute', transform: `translate(-50%, -50%) translate(${labelX}px, ${labelY}px)`, zIndex: 100 }} className="bg-[#0f172a] px-3 py-1 rounded-[0.9rem] border border-white/20 shadow-2xl pointer-events-none">
...
const ConfirmDeleteOverlay: React.FC<{ onConfirm: () => void, onCancel: () => void, label: string }> = ({ onConfirm, onCancel, label }) => (
  <div className="bg-status-error/10 border border-status-error/30 rounded-[0.9rem] p-2 flex flex-col gap-2 animate-apple-in">
    ...
    <div className="flex gap-2">
      <button onClick={(e) => { e.stopPropagation(); onConfirm(); }} className="flex-1 py-1.25 bg-status-error text-white text-[8px] font-black uppercase rounded-[0.9rem] shadow-lg shadow-status-error/20 hover:bg-status-error/80 transition-colors">Confirm Delete</button>
      <button onClick={(e) => { e.stopPropagation(); onCancel(); }} className="flex-1 py-1.25 bg-white/5 text-white/40 text-[8px] font-black uppercase rounded-[0.9rem] hover:bg-white/10 transition-colors">Cancel</button>
    </div>
  </div>
);
...
const Builder2: React.FC<WorkflowBuilderProps> = ({ workflow, taxonomy, relatedWorkflows = [], rollbackPreview, runtimeConfig, onSave, onBack, onExit, onCreateRollbackDraft, onGovernanceAction, setIsDirty }) => {
  ...
  const renderHistorySurface = () => {
    return (
      <div className="space-y-2.5 animate-apple-in pb-8">
        <div className={cn(BUILDER_PANEL, "p-2.5 space-y-2.5")}>
          <div className="flex items-center justify-between gap-2">
            <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(['saved', 'approved', 'selected'] as const).map((mode) => (
              <button
                ...
                className={cn("rounded-[0.9rem] border px-2.5 py-1.25 text-[8px] font-black uppercase tracking-[0.18em] transition-all", historyCompareMode === mode ? "border-theme-accent/20 bg-theme-accent/10 text-theme-accent" : "border-white/10 bg-white/5 text-white/45 hover:text-white")}
              >
            ...
            {historyCompareMode === 'selected' && (
              <select
                className="rounded-[0.9rem] border border-white/10 bg-black/30 px-2.5 py-1.25 text-[8px] font-black uppercase text-white outline-none"
          ...
          <div className="rounded-[0.9rem] border border-white/10 bg-black/20 px-2.5 py-1.5 text-[9px] font-bold text-white/55">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <div className="rounded-[0.9rem] border border-white/10 bg-black/20 p-2.5">
        ...
        <div className="space-y-2.5 max-h-[15rem] overflow-auto pr-1 custom-scrollbar">
          {workflowDiff.addedTasks.length === 0 && workflowDiff.removedTasks.length === 0 && workflowDiff.changedTasks.length === 0 && workflowDiff.addedEdges.length === 0 && workflowDiff.removedEdges.length === 0 && workflowDiff.changedEdges.length === 0 && workflowDiff.changedMetadata.length === 0 && (
            <div className="rounded-[0.9rem] border border-emerald-500/20 bg-emerald-500/10 px-3 py-2.5 text-[10px] font-bold text-emerald-300">
          ...
          {workflowDiff.addedTasks.length > 0 && (
            <div className="space-y-2">
              ...
              {workflowDiff.addedTasks.map((task: any) => (
                <div key={`added-${task.node_id || task.id}`} className="rounded-[0.9rem] border border-emerald-500/20 bg-emerald-500/10 px-3 py-2.5 text-[10px] font-bold text-emerald-100">
                  <div className="flex items-center justify-between gap-2">
              ...
              {workflowDiff.changedTasks.length > 0 && (
                <div className="space-y-2">
                  ...
                  {workflowDiff.changedTasks.map((task: any) => (
                    <div key={`changed-${task.id}`} className="rounded-[0.9rem] border border-amber-500/20 bg-amber-500/10 px-3 py-2.5 text-[10px] font-bold text-amber-100">
                      <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="mt-2 space-y-1">
                    {task.changedFields.map((field: any) => (
                      <div key={`${task.id}-${field.key}`} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-2.5 py-1.5">
          ...
          {workflowDiff.removedTasks.length > 0 && (
            <div className="space-y-2">
              ...
              {workflowDiff.removedTasks.map((task: any) => (
                <div key={`removed-${task.node_id || task.id}`} className="rounded-[0.9rem] border border-rose-500/20 bg-rose-500/10 px-3 py-2.5 text-[10px] font-bold text-rose-100">
          ...
          {workflowDiff.addedEdges.length > 0 || workflowDiff.removedEdges.length > 0 || workflowDiff.changedEdges.length > 0 ? (
            <div className="space-y-2">
              ...
              {workflowDiff.addedEdges.map((edge: any) => (
                <div key={`edge-added-${edge.id}`} className="rounded-[0.9rem] border border-sky-500/20 bg-sky-500/10 px-3 py-2.5 text-[10px] font-bold text-sky-100">
                  New connection: {edge.data?.label || `${edge.source} → ${edge.target}`}
                </div>
              ))}
              {workflowDiff.changedEdges.map((edge: any) => (
                <div key={`edge-${edge.id}`} className="rounded-[0.9rem] border border-sky-500/20 bg-sky-500/10 px-3 py-2.5 text-[10px] font-bold text-sky-100">
                  <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="mt-2 space-y-1">
                    {edge.changedFields.map((field: any) => (
                      <div key={`${edge.id}-${field.key}`} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-2.5 py-1.5">
                        <p className="text-[8px] font-black uppercase tracking-[0.18em] text-white/40">{field.key.replace(/_/g, ' ')}</p>
                        <p className="mt-1 text-[10px] font-bold text-white/70">
                          {formatDiffValue(field.before)} <span className="text-white/25">→</span> {formatDiffValue(field.after)}
                        </p>
              ...
              {workflowDiff.removedEdges.map((edge: any) => (
                <div key={`edge-removed-${edge.id}`} className="rounded-[0.9rem] border border-sky-500/20 bg-sky-500/10 px-3 py-2.5 text-[10px] font-bold text-sky-100">
        ...
        {rollbackPreview?.available && onCreateRollbackDraft && (
          <button onClick={onCreateRollbackDraft} className="w-full rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-2.5 text-[9px] font-black uppercase tracking-[0.18em] text-theme-accent hover:bg-theme-accent hover:text-white transition-all">
    ...
    return (
      <div className="space-y-2.5 animate-apple-in pb-8">
        <div className={cn(BUILDER_PANEL, "p-2.5 space-y-2.5")}>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/55">{openCount} open</span>
          ...
          <textarea
            className="w-full rounded-[0.9rem] border border-white/10 bg-black/30 px-3 py-2.5 text-[11px] font-bold text-white/80 outline-none focus:border-theme-accent min-h-[5.5rem] resize-none"
          ...
          <div className="flex items-center justify-between gap-2.5">
            <button
              ...
              disabled={reviewMode}
              className="rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-2.5 text-[9px] font-black uppercase tracking-[0.18em] text-theme-accent hover:bg-theme-accent hover:text-white transition-all"
            >
              Add Comment
            </button>
        ...
        <div className="space-y-2.5 max-h-[28rem] overflow-auto pr-1 custom-scrollbar">
          {scopedComments.map((comment) => {
            const isEditing = editingCommentId === comment.id;
            return (
              <div key={comment.id} className="rounded-[0.9rem] border border-white/10 bg-black/20 p-2.5 space-y-2.5">
                <div className="flex items-center justify-between gap-2.5">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={reviewMode}
                    className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    disabled={reviewMode}
                    className={cn(
                      "rounded-[0.9rem] border px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em]",
                  ...
                  <button
                    ...
                    disabled={reviewMode}
                    className="rounded-[0.9rem] border border-status-error/20 bg-status-error/10 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-status-error"
                  >
                    Delete
                    </button>
                  </div>
                ...
                {isEditing ? (
                  <div className="space-y-2.5">
                    <textarea
                      className="w-full rounded-[0.9rem] border border-theme-accent/20 bg-black/30 px-3 py-2.5 text-[11px] font-bold text-white/80 outline-none focus:border-theme-accent min-h-[5.5rem] resize-none"
                    ...
                    <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={reviewMode}
                          className="rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-theme-accent"
                        >
                        Save
                      </button>
                        <button
                          ...
                          disabled={reviewMode}
                          className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white/55"
                        >
                        Cancel
                      </button>
                    </div>
          ...
          {scopedComments.length === 0 && (
            <div className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2.5 text-[10px] font-bold text-white/40">
    ...
    <div className="flex h-full min-h-0 w-full bg-[#050914] overflow-hidden">
      {commandPaletteOpen && (
        <div className="fixed inset-0 z-[1100] flex items-start justify-center bg-black/70 backdrop-blur-sm px-3 pt-[12vh]">
          <div className="w-[min(38rem,96vw)] rounded-[0.9rem] border border-white/10 bg-[#0a1120] shadow-2xl">
            <div className="flex items-center gap-2.5 border-b border-white/10 px-3 py-2.5">
              ...
              <button
                ...
                className="rounded-[0.9rem] border border-white/10 bg-white/5 px-2.5 py-1.25 text-[8px] font-black uppercase tracking-[0.18em] text-white/50 hover:text-white"
              >
                Close
              </button>
            </div>
                  ...
                  {actions.map((action) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center justify-between gap-2.5 rounded-[0.9rem] border px-3 py-2 text-left transition-colors",
                          isActive
      ...
      {isOutputPickerOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-8 bg-black/80 backdrop-blur-sm animate-apple-in">
          <div className="w-[min(92vw,52rem)] bg-[#0f172a] border border-white/10 rounded-[0.9rem] shadow-2xl flex flex-col max-h-[88vh] overflow-hidden">
            <div className="p-2 sm:p-2.5 border-b border-white/10 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-2 border-b border-white/10 px-3 py-1.25">
              ...
              />
              {outputPickerSearch && (
                <button onClick={() => setOutputPickerSearch('')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">
                  Clear
                </button>
              )}
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-[#0f172a] z-10 shadow-lg shadow-black/20">
                  <tr className="border-b border-white/10">
                    <th className="px-2.5 py-2 text-[8px] font-black text-white/40 uppercase tracking-widest">Source Task</th>
                    <th className="px-2.5 py-2 text-[8px] font-black text-white/40 uppercase tracking-widest">Output Name</th>
                    <th className="px-2.5 py-2 text-[8px] font-black text-white/40 uppercase tracking-widest">Description</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {tasks
                    ...
                    <tr key={idx} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-2.5 py-2 text-[9px] font-bold text-theme-accent uppercase">{output.taskName}</td>
                      <td className="px-2.5 py-2 text-[9px] font-bold text-white uppercase">{output.name}</td>
                      <td className="px-2.5 py-2 text-[8px] text-white/40 line-clamp-2">{output.description || 'No description'}</td>
                      <td className="px-2.5 py-2 text-right">
                        <button 
                          ...
                          className="px-2 py-1 bg-theme-accent text-white text-[8px] font-black uppercase rounded-[0.9rem] opacity-0 group-hover:opacity-100 hover:scale-105 transition-all"
                        >
                          Select Output
                        </button>
                      </td>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 relative overflow-hidden">
        <div className={cn("min-h-14 border-b border-white/10 bg-[#0a1120]/92 backdrop-blur-xl flex flex-wrap items-center justify-between gap-2 px-3 sm:px-3 py-2 relative z-[100]")}>
          <div className="flex items-center gap-2.5 min-w-0">
            <button onClick={() => onBack(metadata)} className="p-2 hover:bg-white/5 rounded-[0.9rem] transition-colors text-white/40 hover:text-white"><ChevronLeft size={18} /></button>
          ...
          <div className="flex bg-white/5 border border-white/10 rounded-[0.9rem] p-0.5 mr-2 h-[32px] items-center">
          ...
          {!reviewMode && (
            <button disabled={isReadOnlyMode} onClick={() => { if (isReadOnlyMode) return; handleLayout(nodes, edges); }} className={cn(BUILDER_BUTTON, "flex items-center gap-2 px-3 h-[30px] bg-white/5 border border-white/10 text-white uppercase hover:bg-white/10 transition-all", isReadOnlyMode && "opacity-35 cursor-not-allowed")}><RefreshCw size={12} className="text-theme-accent" /> Auto Layout</button>
          )}
          {!reviewMode && (
            <button data-testid="builder-commit" disabled={isReadOnlyMode} onClick={handleSave} className={cn(BUILDER_BUTTON, "flex items-center gap-2 px-3.5 h-[30px] bg-theme-accent text-white shadow-xl shadow-theme-accent/20 hover:scale-[1.02] leading-none", isReadOnlyMode && "opacity-35 cursor-not-allowed")}><Save size={12} /> Commit Changes</button>
          )}
        ...
        <div className="flex-1 relative min-h-0 flex flex-col">
          <div className="shrink-0 border-b border-white/10 bg-[#0a1120]/95 backdrop-blur-xl px-2.5 py-1">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <div className={cn("flex items-center gap-2 rounded-[0.9rem] border px-2.5 py-[5px]", saveStatus === 'conflict' ? "border-status-error/30 bg-status-error/10 text-status-error" : saveStatus === 'saving' ? "border-amber-500/20 bg-amber-500/10 text-amber-300" : saveStatus === 'saved' ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-white/10 bg-white/5 text-white/45")}>
              ...
              {draftRestored && (
                <div className="rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-2.5 py-[5px]">
          ...
          {utilityPane && (
            <div className="shrink-0 border-b border-white/10 bg-[#09111d]/96 backdrop-blur-xl px-3 sm:px-3 py-1.5">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                ...
                <button onClick={closeUtilityPane} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-2.5 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white transition-all leading-none">
                  Close
                </button>
              </div>
              <div className="mt-3">
              ...
              {layoutPrefs.showMiniMap && (
                <div className="absolute bottom-4 right-4 z-20">
                  ...
                  <MiniMap
                    ...
                    className="!bg-[#0a1120] !border !border-white/10 !rounded-[0.9rem] overflow-hidden"
                    pannable
                    zoomable
                  />
                </div>
          ...
          {saveStatus === 'conflict' && saveConflict && (
            <div className="absolute top-[86px] left-4 right-4 z-30 rounded-[0.9rem] border border-status-error/30 bg-status-error/10 px-3 py-2.5 text-[10px] font-bold text-status-error shadow-2xl sm:left-1/2 sm:right-auto sm:-translate-x-1/2">
              <div className="flex flex-wrap items-center justify-between gap-2.5">
                <span>{saveConflict.message || 'Save conflict detected. Reload the latest workflow before saving again.'}</span>
                <button onClick={() => window.location.reload()} className="rounded-[0.9rem] border border-status-error/20 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-status-error">
                  Reload Latest
                </button>
              </div>
            </div>
        ...
        {layoutPrefs.inspectorCollapsed ? (
          <div className="flex-1 min-h-0 overflow-hidden flex items-center justify-center p-2.5">
            <div className="flex flex-col items-stretch gap-2 w-full">
              <button onClick={() => updateLayoutPrefs({ inspectorCollapsed: false })} className="rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-2.5 text-[9px] font-black uppercase tracking-[0.18em] text-theme-accent hover:bg-theme-accent hover:text-white">
                Open Inspector
              </button>
              <button onClick={() => updateLayoutPrefs({ showMiniMap: !layoutPrefs.showMiniMap })} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2.5 text-[9px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">
                {layoutPrefs.showMiniMap ? 'Hide' : 'Show'} MiniMap
              </button>
            </div>
          </div>
        ) : (
          <div ref={taskPaneScrollRef} className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar p-2.5 sm:p-4">
          {selectedTaskId && selectedTask ? (
            <div className={cn(taskPaneCompact ? "space-y-2.5" : "space-y-2.5", "animate-apple-in")}>
              <div className={cn(BUILDER_PANEL, "p-2.5 space-y-2.5")}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/35">
                      <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1">{selectedTask.task_type || 'GENERAL'}</span>
                      {selectedTask.interface && (
                        <span className="rounded-full border border-theme-accent/20 bg-theme-accent/10 px-2.5 py-1 text-theme-accent">{selectedTask.interface}</span>
                      )}
                      {isProtected && (
                        <span className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-1 text-cyan-300">Locked</span>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <div className="min-w-[106px] rounded-[0.9rem] border border-white/10 bg-black/25 px-3 py-2">
                      <div className="flex items-center justify-between gap-2.5">
                    ...
                    <button onClick={() => setTaskPaneCompact((prev) => !prev)} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                      {taskPaneCompact ? 'Expand Pane' : 'Compact Pane'}
                    </button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setUtilityPane('comments')} className="rounded-[0.9rem] border border-theme-accent/20 bg-theme-accent/10 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-theme-accent">
                  ...
                  <button onClick={openHistoryPane} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                    History
                  </button>
                  <button onClick={() => selectedTaskNode && setCenter((selectedTaskNode.position?.x || 0) + 160, (selectedTaskNode.position?.y || 0) + 120, { zoom: 1.05, duration: 250 })} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                    Center
                  </button>
                  <button onClick={() => {
                  ...
                  }} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/55 hover:text-white">
                    Jump Connected
                  </button>
                  <button onClick={() => {
                ...
                {reviewMode && (
                  <div className="rounded-[0.9rem] border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-emerald-300">
                    Review mode is read only. Use the command palette or toggle the button to return to editing.
                  </div>
                )}
                <div className="flex items-center gap-2 rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2">
                  ...
                  {inspectorSearch && (
                    <button
                      ...
                      className="rounded-[0.9rem] border border-white/10 bg-white/5 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white"
                    >
                      Clear
                    </button>
                  )}
                </div>
                <div className="flex flex-wrap gap-2 rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45">
                ...
                <div className="flex flex-wrap items-center gap-2 rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/35">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <button onClick={() => scrollToTaskSection('overview')} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Overview</button>
                  <button onClick={() => scrollToTaskSection('dependencies')} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Dependencies</button>
                  <button onClick={() => scrollToTaskSection('ownership')} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Ownership</button>
                  <button onClick={() => scrollToTaskSection('validation')} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white">Validation</button>
                </div>
                <div className="flex flex-wrap gap-2">
              ...
              {selectedNodeIds.length > 1 && !selectedNodesAreProtected && (
                <div className="rounded-[0.9rem] border border-white/10 bg-white/[0.03] p-2.5 space-y-2.5">
                  <div className="flex items-start justify-between gap-2.5">
                    <div>
                    ...
                    <button onClick={() => { setSelectedNodeIds([]); setSelectedTaskId(null); }} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/45">Clear</button>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-2.5">
                    <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] text-white outline-none focus:border-theme-accent" placeholder="Bulk owner team" value={selectedTask.owning_team || ''} onChange={e => updateSelectedTasks({ owning_team: e.target.value })} />
                    <select className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] font-black text-white outline-none focus:border-theme-accent" value={selectedTask.task_type} onChange={e => updateSelectedTasks({ task_type: e.target.value })}>
                      {taskTypes.map((type: any) => <option key={type} value={type}>{type}</option>)}
                    </select>
                    <button onClick={() => updateSelectedTasks({ validation_needed: !selectedTask.validation_needed })} className={cn("rounded-[0.9rem] border px-3 py-2.5 text-[10px] font-black uppercase transition-all", selectedTask.validation_needed ? "border-orange-500/30 bg-orange-500/10 text-orange-400" : "border-white/10 bg-white/5 text-white/45 hover:text-white")}>
                      Toggle Validation
                    </button>
                    <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] text-white outline-none focus:border-theme-accent" placeholder="Bulk backup owner" value={selectedTaskDiagnostics.taskManagement?.backup_owner || ''} onChange={e => updateSelectedTasksDiagnostics({ taskManagement: { backup_owner: e.target.value } })} />
                    <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] text-white outline-none focus:border-theme-accent" placeholder="Bulk reviewer" value={selectedTaskDiagnostics.taskManagement?.reviewer || ''} onChange={e => updateSelectedTasksDiagnostics({ taskManagement: { reviewer: e.target.value } })} />
                    <input type="number" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] text-white outline-none focus:border-theme-accent" placeholder="Bulk manual minutes" value={selectedTask.manual_time_minutes || 0} onChange={e => updateSelectedTasks({ manual_time_minutes: parseFloat(e.target.value) || 0 })} />
                    <input type="number" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2.5 text-[11px] text-white outline-none focus:border-theme-accent" placeholder="Bulk automation minutes" value={selectedTask.automation_time_minutes || 0} onChange={e => updateSelectedTasks({ automation_time_minutes: parseFloat(e.target.value) || 0 })} />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => alignSelectedNodes('left')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/50 hover:text-white">Align Left</button>
                    <button onClick={() => alignSelectedNodes('top')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/50 hover:text-white">Align Top</button>
                    <button onClick={() => alignSelectedNodes('center')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/50 hover:text-white">Center X</button>
                    <button onClick={() => alignSelectedNodes('horizontal')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/50 hover:text-white">Line Up</button>
                    <button onClick={() => alignSelectedNodes('vertical')} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/50 hover:text-white">Stack</button>
                  </div>
                </div>
              )}
              {inspectorTab === 'overview' && (
                <div ref={(el) => { taskSectionRefs.current.overview = el; }} className={cn(taskPaneCompact ? "space-y-2.5" : "space-y-6", isReadOnlyMode && "pointer-events-none select-none opacity-80")} data-section="overview">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className={cn("w-full bg-white/5 border rounded-[0.9rem] px-3 py-2 text-[12px] font-bold text-white outline-none focus:border-theme-accent disabled:opacity-50 disabled:cursor-not-allowed", issuesForField('task.name', selectedTaskId).length > 0 ? "border-status-error/40 bg-status-error/10" : "border-white/10")} 
                    ...
                    {issuesForField('task.description', selectedTaskId).length > 0 && (
                      <div className="space-y-2">
                        {issuesForField('task.description', selectedTaskId).map((issue) => (
                          <div key={issueId(issue)} className="rounded-[0.9rem] border border-status-error/30 bg-status-error/10 px-3 py-2.5 text-[10px] font-bold text-status-error">
                            {issue.message}
                          </div>
                        ))}
                      </div>
                  ...
                  <div
                    ...
                    className={cn("rounded-[0.9rem] border p-2.5 space-y-2.5", selectedTaskDiff.changedFields.length > 0 ? "border-theme-accent/20 bg-theme-accent/5" : "border-white/5 bg-white/[0.02]")}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      <div className="rounded-[0.9rem] border border-white/10 bg-black/20 p-2.5 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedTaskIncomingTasks.length > 0 ? selectedTaskIncomingTasks.map((task) => (
                            <button
                              ...
                              className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[9px] font-black uppercase text-white/60 hover:text-white"
                            >
                              {task.name || 'Untitled'}
                            </button>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      <div className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2">
                  ...
                  <div className="rounded-[0.9rem] border border-white/5 bg-white/[0.02] p-2.5 space-y-2.5">
                    <div className="flex items-center justify-between gap-2.5">
                      <div>
                    ...
                    {selectedTaskDiff.changedFields.length === 0 ? (
                      <div className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2.5 text-[11px] font-bold text-white/35">
                        No task-level field changes detected.
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {selectedTaskDiff.changedFields.slice(0, 4).map((field) => (
                          <div key={field.key} className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2">
                            <p className="text-[8px] font-black uppercase tracking-[0.18em] text-white/40">{field.key.replace(/_/g, ' ')}</p>
                            <p className="mt-1 text-[10px] font-bold text-white/70">
                              {formatDiffValue(field.before)} <span className="text-white/25">→</span> {formatDiffValue(field.after)}
                            </p>
                  ...
                  {!isProtected && selectedTask.task_type !== 'LOOP' && (
                    <>
                      ...
                      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 p-2.5 bg-white/[0.02] border border-white/5 rounded-[0.9rem]">
                        <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 items-end">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="rounded-[0.9rem] border border-white/10 bg-black/30 p-2">
                            <button
                              onClick={() => setOwnerPositionsCollapsed(!ownerPositionsCollapsed)}
                              className="w-full rounded-[0.9rem] border border-white/10 bg-white/[0.03] px-3 h-10 flex items-center justify-between hover:bg-white/10 transition-colors"
                            >
                      ...
                      <div
                        ...
                        className="rounded-[0.9rem] border border-white/5 bg-white/[0.02] p-2.5 space-y-2.5"
                      ...
                      >
                        ...
                        <div className={cn(taskPaneCompact ? "grid grid-cols-1 sm:grid-cols-2 gap-2" : "grid grid-cols-1 sm:grid-cols-2 gap-2.5")}>
                      ...
                      <div className="space-y-2.5">
                      <div className="flex items-center justify-between gap-2.5 px-1" ref={(el) => { taskSectionRefs.current.systems = el; }} data-section="systems">
                        <div>
                          ...
                          <button
                            ...
                            className={cn("rounded-[0.9rem] border px-3 py-2 text-[8px] font-black uppercase tracking-[0.18em]", selectedTaskDiagnostics.taskProfile?.shadow_it_used ? "border-amber-500/20 bg-amber-500/10 text-amber-300" : "border-white/10 bg-white/5 text-white/45 hover:text-white")}
                        >
                          Shadow IT {selectedTaskDiagnostics.taskProfile?.shadow_it_used ? 'On' : 'Off'}
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2 px-1">
                        <span className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-[8px] font-black uppercase tracking-[0.18em] text-white/45">
                      ...
                      {selectedTaskDiagnostics.taskProfile?.shadow_it_used && (
                            ...
                            >
                              <div className="space-y-2.5">
                                <div className="space-y-1">
                                  ...
                                  <input data-builder-field="task.involved_systems" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white outline-none focus:border-theme-accent" value={sys.name} onChange={e => updateTask(selectedTaskId, { involved_systems: selectedTask.involved_systems.map(x => x.id === sys.id ? { ...x, name: e.target.value } : x) })} placeholder="e.g., SAP, Salesforce, Internal Tool" />
                                </div>
                                <div className="space-y-1">
                                  <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Usage Context</label>
                                  <textarea className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/60 h-[4.5rem] resize-none outline-none focus:border-theme-accent" value={sys.usage} onChange={e => updateTask(selectedTaskId, { involved_systems: selectedTask.involved_systems.map(x => x.id === sys.id ? { ...x, usage: e.target.value } : x) })} placeholder="Describe how the system is used in this task..." />
                                </div>
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-[0.9rem] px-3 py-2">
                        ...
                        <div className="rounded-[0.9rem] border border-white/10 bg-black/20 px-3 py-2.5 text-[10px] font-bold text-white/35">
                          ...
                          <button onClick={() => updateTask(selectedTaskId, { involved_systems: [...(selectedTask.involved_systems || []), { id: Date.now().toString(), name: '', usage: '', figures: [], link: '' }] })} className="w-full py-2.5 bg-white/5 border border-dashed border-white/10 rounded-[0.9rem] text-[9px] font-black uppercase text-white/40 hover:text-white hover:bg-white/10 transition-all">+ Add System Dependency</button>
                        </div>
                      </div>
                      ...
                      >
                        ...
                        <div className="space-y-2 max-h-48 overflow-auto pr-1 custom-scrollbar">
                          {selectedTaskComments.slice(0, 3).map((comment) => (
                            <div key={comment.id} className="rounded-[0.9rem] border border-white/10 bg-black/20 p-2.5 space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                <div>
                                ...
                                <button onClick={() => toggleWorkflowCommentStatus(comment.id)} disabled={reviewMode} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white/50">
                                  {comment.status === 'resolved' ? 'Reopen' : 'Resolve'}
                                </button>
                              </div>
                              <p className="text-[11px] font-bold text-white/70 leading-relaxed whitespace-pre-wrap">{comment.message}</p>
                    ...
                    <div className="pt-4 border-t border-white/5 space-y-2.5">
                      {confirmingDelete === selectedTaskId ? (
                        ...
                        <button 
                          ...
                          className="w-full py-2.5 bg-status-error/10 border border-status-error/20 text-status-error rounded-[0.9rem] text-[10px] font-black uppercase hover:bg-status-error hover:text-white transition-all flex items-center justify-center gap-2"
                        >
              ...
              {inspectorTab === 'data' && (
                <div className={cn("space-y-2.5 animate-apple-in", isReadOnlyMode && "pointer-events-none select-none opacity-80")}>
                  <CollapsibleSection title="Task Inputs" isOpen={expandedSections.inputs} toggle={() => toggleSection('inputs')} count={selectedTask.source_data_list.length}>
                    <div className="space-y-2.5 pt-4">
                        ...
                        >
                          <div className="space-y-2.5">
                            ...
                            <div className="space-y-1">
                              ...
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-theme-accent outline-none disabled:opacity-50" value={sd.link} onChange={e => updateTask(selectedTaskId, { source_data_list: selectedTask.source_data_list.map(x => x.id === sd.id ? { ...x, link: e.target.value } : x) })} placeholder="Relevant URL" disabled={!!sd.from_task_id} />
                            </div>
                          </div>
                        </NestedCollapsible>
                      ))}
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <button onClick={() => updateTask(selectedTaskId, { source_data_list: [...selectedTask.source_data_list, { id: Date.now().toString(), name: '', description: '', figures: [], link: '', data_example: '' }] })} className="py-2.5 bg-white/5 border border-white/10 text-[9px] font-black uppercase text-white/40 hover:text-white hover:bg-white/10 transition-all rounded-[0.9rem] flex items-center justify-center gap-2"><Plus size={12} /> Add Manual Input</button>
                        <button onClick={() => setIsOutputPickerOpen(true)} className="py-2.5 bg-theme-accent/10 border border-theme-accent/20 text-[9px] font-black uppercase text-theme-accent hover:bg-theme-accent hover:text-white transition-all rounded-[0.9rem] flex items-center justify-center gap-2"><Search size={12} /> Registry Search</button>
                      </div>
                    </div>
                  </CollapsibleSection>
                  ...
                  <CollapsibleSection title="Task Outputs" isOpen={expandedSections.outputs} toggle={() => toggleSection('outputs')} count={selectedTask.output_data_list.length}>
                    <div className="space-y-2.5 pt-4">
                        ...
                        >
                          <div className="space-y-2.5">
                            <div className="space-y-1">
                              ...
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white outline-none focus:border-theme-accent" value={od.name} onChange={e => updateTask(selectedTaskId, { output_data_list: selectedTask.output_data_list.map(x => x.id === od.id ? { ...x, name: e.target.value } : x) })} placeholder="e.g., Final Report, Updated DB Entry" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Description</label>
                              <textarea className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/60 h-[4.5rem] resize-none outline-none focus:border-theme-accent" value={od.description} onChange={e => updateTask(selectedTaskId, { output_data_list: selectedTask.output_data_list.map(x => x.id === od.id ? { ...x, description: e.target.value } : x) })} placeholder="Define the output artifact..." />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Format / Example</label>
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/60 outline-none focus:border-theme-accent" value={od.data_example} onChange={e => updateTask(selectedTaskId, { output_data_list: selectedTask.output_data_list.map(x => x.id === od.id ? { ...x, data_example: e.target.value } : x) })} placeholder="Example value or format" />
                            </div>
                            ...
                            <div className="space-y-1">
                              <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Links</label>
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-theme-accent outline-none" value={od.link} onChange={e => updateTask(selectedTaskId, { output_data_list: selectedTask.output_data_list.map(x => x.id === od.id ? { ...x, link: e.target.value } : x) })} placeholder="Relevant URL" />
                            </div>
                          </div>
                        </NestedCollapsible>
                      ))}
                      ...
                      <button onClick={() => updateTask(selectedTaskId, { output_data_list: [...selectedTask.output_data_list, { id: Date.now().toString(), name: '', description: '', figures: [], link: '', data_example: '' }] })} className="w-full py-2.5 bg-white/5 border border-white/10 text-[10px] font-black uppercase text-white/40 hover:text-white hover:bg-white/10 transition-all rounded-[0.9rem] flex items-center justify-center gap-2"><Plus size={12} /> Add Output Artifact</button>
                    </div>
                  </CollapsibleSection>
                </div>
              )}
              {inspectorTab === 'exceptions' && (
                <div className={cn("space-y-2.5", isReadOnlyMode && "pointer-events-none select-none opacity-80")}>
                  <CollapsibleSection title="Operational Roadblocks" isOpen={expandedSections.blockers} toggle={() => toggleSection('blockers')} count={selectedTask.blockers.length}>
                    <div className="space-y-2.5 pt-4">
                        ...
                        >
                          <div className="space-y-2.5">
                            <div className="space-y-1">
                              ...
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white outline-none focus:border-amber-500" value={b.blocking_entity} onChange={e => updateTask(selectedTaskId, { blockers: selectedTask.blockers.map(x => x.id === b.id ? { ...x, blocking_entity: e.target.value } : x) })} placeholder="What stops the process?" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Average Delay (Minutes)</label>
                              <input type="number" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[12px] text-white" value={b.average_delay_minutes || 0} onChange={e => updateTask(selectedTaskId, { blockers: selectedTask.blockers.map(x => x.id === b.id ? { ...x, average_delay_minutes: parseFloat(e.target.value) || 0 } : x) })} />
                            </div>
                            <div className="space-y-2.5">
                            ...
                            <div className="space-y-1">
                              ...
                              <textarea className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/60 h-[4.5rem] resize-none outline-none focus:border-amber-500" value={b.reason} onChange={e => updateTask(selectedTaskId, { blockers: selectedTask.blockers.map(x => x.id === b.id ? { ...x, reason: e.target.value } : x) })} placeholder="Action to reduce delay..." />
                            </div>
                          </div>
                        </NestedCollapsible>
                      ))}
                      ...
                      <button onClick={() => updateTask(selectedTaskId, { blockers: [...selectedTask.blockers, { id: Date.now().toString(), blocking_entity: '', reason: '', average_delay_minutes: 0, frequency: 1 }] })} className="w-full py-2.5 bg-amber-500/10 border border-amber-500/20 text-[10px] font-black uppercase text-amber-500 hover:bg-amber-500/20 transition-all rounded-[0.9rem] flex items-center justify-center gap-2"><Plus size={12} /> Add Roadblock</button>
                    </div>
                  </CollapsibleSection>
                        ...
                        >
                          <div className="space-y-2.5">
                            <div className="space-y-1">
                              ...
                              <input className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white outline-none focus:border-status-error" value={er.error_type} onChange={e => updateTask(selectedTaskId, { errors: selectedTask.errors.map(x => x.id === er.id ? { ...x, error_type: e.target.value } : x) })} placeholder="Describe the common human error" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[9px] font-black text-white/20 uppercase tracking-widest">Recovery (Minutes)</label>
                              <input type="number" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[12px] text-white" value={er.recovery_time_minutes || 0} onChange={e => updateTask(selectedTaskId, { errors: selectedTask.errors.map(x => x.id === er.id ? { ...x, recovery_time_minutes: parseFloat(e.target.value) || 0 } : x) })} />
                            </div>
                            <div className="space-y-2.5">
                              <div className="flex justify-between items-center">
                            ...
                            <div className="space-y-1">
                              ...
                              <textarea 
                                className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/60 h-[5.5rem] resize-none outline-none focus:border-status-error" 
              ...
              {inspectorTab === 'validation' && (
                <div ref={(el) => { taskSectionRefs.current.validation = el; }} className={cn("space-y-2.5 animate-apple-in", isReadOnlyMode && "pointer-events-none select-none opacity-80")} data-section="validation">
                  <div className="p-2.5 bg-white/[0.02] border border-white/5 rounded-[0.9rem] space-y-2.5">
                    <div className="flex items-center justify-between gap-2.5">
                      <div className="space-y-1">
                    ...
                    {selectedTask.validation_needed && (
                      <div className="space-y-2.5 animate-apple-in pt-3 border-t border-white/5">
                        ...
                        >
                            <div className="space-y-2.5">
                              <div className="space-y-1">
                                ...
                                className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/80 h-24 resize-none outline-none focus:border-orange-500" 
                                ...
                                {!step.description && issuesForField('task.validation_step', selectedTaskId).length > 0 && (
                                  <p className={cn("mt-2 rounded-[0.9rem] border px-3 py-2 text-[10px] font-bold leading-relaxed", compactIssueTone('error'))}>
                  ...
                  <div className="rounded-[0.9rem] border border-white/10 bg-black/20 p-2.5 space-y-2">
                    <div className="flex items-center justify-between gap-2.5">
                      ...
                      <button onClick={() => blockingValidationIssues[0] && focusAuditIssue(blockingValidationIssues[0])} className="rounded-[0.9rem] border border-white/10 bg-white/5 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] text-white/45 hover:text-white leading-none">
                        Jump to first issue
                      </button>
                    </div>
              ...
              {inspectorTab === 'appendix' && (
                <div className={cn("space-y-2.5 pb-16", isReadOnlyMode && "pointer-events-none select-none opacity-80")}>
                  <CollapsibleSection title="Operational References" isOpen={expandedSections.references} toggle={() => toggleSection('references')} count={selectedTask.reference_links.length}>
                    <div className="space-y-2.5 pt-4">
                        ...
                        >
                          <div className="space-y-2.5">
                            <div className="space-y-1">
                              ...
                              <input data-builder-field="task.reference_links" className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white outline-none focus:border-theme-accent" value={l.label} onChange={e => updateTask(selectedTaskId, { reference_links: selectedTask.reference_links.map(x => x.id === l.id ? { ...x, label: e.target.value } : x) })} placeholder="e.g., SOP v1.2" />
                            </div>
                            <div className="space-y-1">
                      ...
                      <button onClick={() => updateTask(selectedTaskId, { reference_links: [...selectedTask.reference_links, { id: Date.now().toString(), label: '', url: '' }] })} className="w-full py-2.5 bg-white/5 border border-dashed border-white/10 text-[9px] font-black uppercase text-white/40 hover:text-white transition-all rounded-[0.9rem] mt-2">+ Add Reference Link</button>
                    </div>
                  </CollapsibleSection>
                  ...
                  <CollapsibleSection title="Step-by-Step Instructions" isOpen={expandedSections.instructions} toggle={() => toggleSection('instructions')} count={selectedTask.instructions.length}>
                    <div className="space-y-2.5 pt-4">
                          ...
                          <div className="space-y-2.5">
                            <div className="space-y-1">
                              ...
                              <textarea 
                                ...
                                className="w-full bg-black/40 border border-white/10 rounded-[0.9rem] p-2.5 text-[11px] text-white/80 h-28 resize-none outline-none focus:border-theme-accent transition-all" 
                                value={step.description} 
                      ...
                      <button onClick={() => updateTask(selectedTaskId, { instructions: [...selectedTask.instructions, { id: Date.now().toString(), description: '', figures: [], links: [] }] })} className="w-full py-2.5 bg-white/5 border border-dashed border-white/10 text-[9px] font-black uppercase text-white/40 hover:text-white transition-all rounded-[0.9rem]">+ Add Instruction Step</button>
                    </div>
                  </CollapsibleSection>
                </div>
              )}
            ...
            <div className={cn("p-2.5 space-y-2.5 animate-apple-in", isReadOnlyMode && "pointer-events-none select-none opacity-80")}>
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <div className="flex items-center gap-2.5"><Link2 size={16} className="text-theme-accent" /><span className="text-[14px] font-black text-white uppercase tracking-widest">Edge Configuration</span></div>
                <div className="flex gap-2">
                  <button onClick={() => centerOnEdgeRoute(selectedEdgeId)} title="Center Route" className="text-white/40 hover:text-white p-2 bg-white/5 border border-white/10 rounded-[0.9rem] transition-all leading-none"><MapPinned size={16} /></button>
                  <button onClick={() => swapEdgeDirection(selectedEdgeId)} title="Swap Direction" className="text-white/40 hover:text-white p-2 bg-white/5 border border-white/10 rounded-[0.9rem] transition-all leading-none"><LucideWorkflow size={16} className="rotate-90" /></button>
                  <button onClick={() => { if (isReadOnlyMode) return; setEdges(eds => eds.filter(e => e.id !== selectedEdgeId)); setSelectedEdgeId(null); setIsDirty?.(true); }} className="text-status-error hover:bg-status-error/10 p-2 border border-status-error/20 rounded-[0.9rem] transition-all leading-none"><Trash size={16} /></button>
                </div>
              </div>
              <div className="rounded-[0.9rem] border border-white/10 bg-white/[0.03] p-2.5 space-y-2">
                ...
                <div className="flex flex-wrap gap-2 text-[11px] font-bold text-white/65">
                  <span className="rounded-[0.9rem] border border-white/10 bg-black/25 px-3 py-2">From: {nodes.find((node) => node.id === selectedEdge.source)?.data?.label || selectedEdge.source}</span>
                  <span className="rounded-[0.9rem] border border-white/10 bg-black/25 px-3 py-2">To: {nodes.find((node) => node.id === selectedEdge.target)?.data?.label || selectedEdge.target}</span>
                  <span className="rounded-[0.9rem] border border-white/10 bg-black/25 px-3 py-2">Style: {selectedEdge.data?.edgeStyle || 'bezier'}</span>
                  <span className="rounded-[0.9rem] border border-white/10 bg-black/25 px-3 py-2">Line: {selectedEdge.data?.lineStyle || 'solid'}</span>
                </div>
              </div>
              <div className="space-y-2.5">
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-white/40 uppercase px-1">Label</label>
                  <input data-builder-field="edge.label" className="w-full bg-white/5 border border-white/10 rounded-[0.9rem] px-3 py-2 text-[11px] font-black text-white uppercase outline-none focus:border-theme-accent transition-all" value={selectedEdge.data?.label || ''} onChange={e => updateEdge(selectedEdgeId, { label: e.target.value })} />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-white/40 uppercase px-1">Style</label>
                  <div className="flex bg-white/5 p-1 rounded-[0.9rem] border border-white/10">
                    {(['smoothstep', 'bezier', 'straight'] as const).map((s) => (
                      <button key={s} disabled={isReadOnlyMode} onClick={() => { if (isReadOnlyMode) return; updateEdge(selectedEdgeId, { edgeStyle: s }); }} className={cn("flex-1 py-2 text-[9px] font-black uppercase rounded-[0.9rem] transition-all leading-none", isReadOnlyMode ? "opacity-35 cursor-not-allowed" : (selectedEdge.data?.edgeStyle || 'bezier') === s ? "bg-theme-accent text-white" : "text-white/40 hover:text-white")}>{s}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex bg-white/5 p-1 rounded-[0.9rem] border border-white/10">
                    {(['solid', 'dashed'] as const).map((s) => (
                      <button key={s} disabled={isReadOnlyMode} onClick={() => { if (isReadOnlyMode) return; updateEdge(selectedEdgeId, { lineStyle: s }); }} className={cn("flex-1 py-2 text-[9px] font-black uppercase rounded-[0.9rem] transition-all leading-none", isReadOnlyMode ? "opacity-35 cursor-not-allowed" : (selectedEdge.data?.lineStyle || 'solid') === s ? "bg-theme-accent text-white" : "text-white/40 hover:text-white")}>{s}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-2">
            ...
            <div className={cn("space-y-2.5 animate-apple-in pb-14", isReadOnlyMode && "pointer-events-none select-none opacity-80")}>
              ...
              <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    disabled={reviewMode}
                    className={cn("px-3.5 py-[5px] rounded-[0.9rem] text-[8px] font-black uppercase transition-all leading-none", reviewMode ? "hidden bg-white/5 text-white/25 cursor-not-allowed" : isMetadataEditMode ? "bg-theme-accent text-white" : "bg-white/5 text-white/40 hover:text-white")}
                  >
              ...
              <div className={cn(definitionCompactMode ? "space-y-2.5" : "space-y-2.5", "transition-all", (!isMetadataEditMode || reviewMode) && "opacity-80 pointer-events-none")}>
                ...
                <div className={cn(BUILDER_PANEL, definitionCompactMode ? "space-y-2 !bg-white/[0.02] border-white/5 p-2.5" : "space-y-2.5 !bg-white/[0.02] border-white/5 p-2.5")}>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className={cn(BUILDER_PANEL, definitionCompactMode ? "space-y-2.5 !bg-white/[0.02] border-white/5 p-2.5" : "space-y-2.5 !bg-white/[0.02] border-white/5 p-2.5")}>
                    <div className={cn("grid grid-cols-1 lg:grid-cols-2 gap-2", definitionCompactMode && "gap-1.5")}>
                      ...
                      <div className={cn("grid grid-cols-1 gap-2.5 border-t border-white/5 pt-4", definitionCompactMode && "gap-2.5 pt-3")}>
                      <div className="space-y-1.5">
